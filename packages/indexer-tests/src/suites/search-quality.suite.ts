...
export function runSearchQualitySuite(getIndexer: () => Indexer): void {
  describe("Search Quality", () => {
    ...
    it("FTS Hit@1 >= 40% and Hit@3 >= 60%", async () => {
      ...
      expect(rate1, `FTS Hit@1 = ${(rate1 * 100).toFixed(0)}%`).toBeGreaterThanOrEqual(0.4);
      expect(rate3, `FTS Hit@3 = ${(rate3 * 100).toFixed(0)}%`).toBeGreaterThanOrEqual(0.6);
    });
    ...
    it("Vector Hit@1 >= 30% and Hit@3 >= 50%", async () => {
      ...
      const rate1 = hit1 / queries.length;
      const rate3 = hit3 / queries.length;
      expect(rate1, `Vec Hit@1 = ${(rate1 * 100).toFixed(0)}%`).toBeGreaterThanOrEqual(0.3);
      expect(rate3, `Vec Hit@3 = ${(rate3 * 100).toFixed(0)}%`).toBeGreaterThanOrEqual(0.5);
    });
    ...
    it("Hybrid Hit@3 >= max(FTS, Vector) Hit@3", async () => {
      ...
      for (const q of queries) {
        ...
        if (ftsResults.slice(0, 3).some((r) => blockIdToFile.get(r.blockId) === q.expectedTopPath))
          ftsHit3++;
        ...
        if (vecResults.slice(0, 3).some((r) => blockIdToFile.get(r.blockId) === q.expectedTopPath))
          vecHit3++;
        ...
        if (
          hybridResults.slice(0, 3).some((r) => blockIdToFile.get(r.blockId) === q.expectedTopPath)
        )
          hybridHit3++;
      }
