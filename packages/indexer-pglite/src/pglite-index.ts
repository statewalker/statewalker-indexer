...
export class PGLiteIndex implements Index {
  ...
  private readonly fts: PGLiteFullTextIndex | null;
  private readonly vec: PGLiteVectorIndex | null;
  ...
  async *search(params: HybridSearchParams): AsyncGenerator<HybridSearchResult> {
    ...
    const hasEmbeddings = embeddings && embeddings.length > 0 && this.vec !== null;
  ...
  async getSize(pathPrefix?: DocumentPath): Promise<number> {
    ...
    if (hasFts && hasVec) {
      ...
      const ftsTable = (this.fts as PGLiteFullTextIndex).tableName;
      const vecTable = (this.vec as PGLiteVectorIndex).tableName;
      const sql = `SELECT COUNT(*) AS cnt FROM (SELECT b.doc_id, b.block_id FROM ${ftsTable} b JOIN ${this.docsTable} d ON d.doc_id = b.doc_id${pathClause} UNION SELECT b.doc_id, b.block_id FROM ${vecTable} b JOIN ${this.docsTable} d ON d.doc_id = b.doc_id${pathClause}) AS combined`;
