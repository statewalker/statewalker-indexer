...
export class PGLiteVectorIndex implements EmbeddingIndex {
  ...
  constructor(db: PGlite, prefix: string, docsTable: string, info: EmbeddingIndexInfo) {
    this.db = db;
  ...
  async init(): Promise<void> {
    ...
    await this.db.exec(
      `CREATE TABLE IF NOT EXISTS ${this.tableName} (doc_id INTEGER NOT NULL, block_id TEXT NOT NULL, embedding vector(${dim}) NOT NULL, PRIMARY KEY (doc_id, block_id))`,
    );
    await this.db.exec(
      `CREATE INDEX IF NOT EXISTS ${this.indexName} ON ${this.tableName} USING hnsw (embedding vector_cosine_ops)`,
    );
  }
  ...
  async *search(params: EmbeddingSearchParams): AsyncGenerator<EmbeddingSearchResult> {
    ...
    for (const queryEmb of embeddings) {
      ...
      const { rows } = await this.db.query<{
        ...
        `SELECT d.path, b.block_id, (b.embedding <=> $1::vector(${dim})) AS dist FROM ${this.tableName} b JOIN ${this.docsTable} d ON d.doc_id = b.doc_id ${pathClause}ORDER BY dist ASC LIMIT ${topKParam}`,
        queryParams,
      );
  ...
  async addDocument(blocks: EmbeddingBlock[]): Promise<void> {
    ...
    for (const block of blocks) {
      ...
      await this.db.query(
        `INSERT INTO ${this.tableName} (doc_id, block_id, embedding) VALUES ($1, $2, $3::vector(${dim}))`,
  ...
  async *getDocumentsBlocks(pathPrefix?: DocumentPath): AsyncGenerator<EmbeddingBlock> {
    ...
    for (const row of rows) {
      // PGlite returns vector as string "[1,2,3]", parse to Float32Array
