...
export class MemIndex implements Index {
  ...
  private readonly trackedBlocks = new Map<string, { path: DocumentPath; blockId: string }>();
    ...
    for (const entry of this.trackedBlocks.values()) {
      ...
      if (this.fts !== null) {
        for await (const ftsBlock of this.fts.getDocumentsBlocks()) {
          if (ftsBlock.path === entry.path && ftsBlock.blockId === entry.blockId) {
      ...
      if (this.vec !== null) {
        for await (const vecBlock of this.vec.getDocumentsBlocks()) {
          if (vecBlock.path === entry.path && vecBlock.blockId === entry.blockId) {
