...
} from "../../src/utils/query-parser.js";
...
describe("parseStructuredQuery", () => {
  ...
  describe("error cases", () => {
    it("mixed plain + prefixed throws", () => {
      expect(() => parseStructuredQuery("plain text\nlex: keyword")).toThrowError(/mix/i);
    });

    it("expand: mixed with typed throws", () => {
      expect(() => parseStructuredQuery("expand: something\nlex: keyword")).toThrowError(/mix/i);
    });
...
describe("validateSemanticQuery", () => {
  ...
  it("accepts hypothetical passage — returns null", () => {
    expect(
      validateSemanticQuery("A hypothetical passage about distributed consensus algorithms"),
    ).toBeNull();
  });
});
