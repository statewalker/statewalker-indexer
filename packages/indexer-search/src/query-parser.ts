...
export function parseStructuredQuery(input: string): ParsedQuery[] | null {
  ...
  for (const { trimmed, lineNumber } of entries) {
    ...
    if (typedMatch) {
      ...
      if (rest.length === 0) {
        throw new Error(`Empty query after "${prefix}:" prefix on line ${lineNumber}`);
      }
    ...
    if (expandMatch) {
      ...
      if (rest.length === 0) {
        throw new Error(`Empty query after "expand:" prefix on line ${lineNumber}`);
      }
      hasExpand = true;
      continue;
    }
  ...
  if (plain.length > 0 && typed.length > 0) {
    throw new Error("Cannot mix plain text with typed (lex:/vec:/hyde:) queries");
  }
