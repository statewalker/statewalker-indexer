...
export function runDocumentPathsSuite(getIndexer: () => Indexer): void {
  describe("Document Paths", () => {
    ...
    it("search without paths filter returns all documents", async () => {
      ...
      await index.addDocument([
        { path: "/a/1", blockId: "1", content: "hello world" },
      ]);
      await index.addDocument([
        { path: "/b/2", blockId: "2", content: "hello earth" },
      ]);
    ...
    it("search with multiple path prefixes returns union", async () => {
      ...
      await index.addDocument([
        { path: "/science/physics", blockId: "1", content: "atoms" },
      ]);
      await index.addDocument([
        { path: "/tech/code", blockId: "2", content: "atoms in code" },
      ]);
      await index.addDocument([
        { path: "/art/music", blockId: "3", content: "atomic beats" },
      ]);
    ...
    it("getSize with pathPrefix counts only matching blocks", async () => {
      ...
      await index.addDocument([
        { path: "/docs/guide", blockId: "3", content: "tutorial" },
      ]);
      await index.addDocument([
        { path: "/blog/post1", blockId: "4", content: "hello" },
      ]);
    ...
    it("getDocumentPaths yields all unique paths", async () => {
      ...
      await index.addDocument([
        { path: "/docs/b", blockId: "3", content: "third" },
      ]);
    ...
    it("getDocumentPaths with pathPrefix yields only matching", async () => {
      ...
      await index.addDocument([
        { path: "/blog/b", blockId: "2", content: "second" },
      ]);
    ...
    it("FTS sub-index respects path prefix in search", async () => {
      ...
      await fts.addDocument([
        { path: "/science/1", blockId: "1", content: "quantum physics" },
      ]);
      await fts.addDocument([
        { path: "/tech/1", blockId: "2", content: "quantum computing" },
      ]);
